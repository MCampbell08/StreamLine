...
namespace StreamLine.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser
                {
                    ...
                    Email = model.Email,
                    TwitterConnect = model.TwitterConnect,
                    InstagramConnect = model.InstagramConnect,
                    SpotifyPlaylistUri = "https://embed.spotify.com/?uri=" + model.SpotifyPlaylistURI + "&theme=white",
                    AboutMe = model.AboutMe,
                    BirthDate = model.BirthDate.ToString(),
                    FirstName = model.FirstName, 
                    LastName = model.LastName
                };
                if (user.SpotifyPlaylistUri == null)
                {
                    user.SpotifyPlaylistUri = "https://embed.spotify.com/?uri=spotify:user:spotify:playlist:0IXz49HvVcdqSMKGolunm1&theme=white";
                }
        ...
        public ActionResult AddFriend(string[] friend)
        {
            ...
            return RedirectToAction("ProfilePageView");
        }
        ...
        public ActionResult UserProfile(string friend)
        {
            StreamLineDataEntities context = new StreamLineDataEntities();

            var user = context.AspNetUsers.Single(u => u.Email == friend);
        ...
        public ActionResult RemoveFriend(string friend)
        {
            ...
            foreach(char c in user.Friends)
            {
                if (c == '&')
                {
                    friendsList.Add(sampleFriend);
                    sampleFriend = "";
                }
                else
                {
            ...
            {
                sampleFriend += friendName + "&";
            }
            user.Friends = sampleFriend;
            context.Entry(user).State = System.Data.Entity.EntityState.Modified;
        ...
        public AccountController EditProfile()
        {
            StreamLineDataEntities context = new StreamLineDataEntities();
            var user = context.AspNetUsers.Single(u => u.Email == User.Identity.Name);
